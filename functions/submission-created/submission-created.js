...
const pipedriveUpdateFunc = async (process, data) => {
  // handle form contact
...
const klaviyoFunc = async (process, data) => {
...
const klaviyoNewsLetterFunc = async (process, data) => {
  const { email } = data;
...
const pipedriveFunc = async (process, data) => {
  const { PIPEDRIVE_USER_ID, PIPEDRIVE_KEY } = process.env;
  const fields = {
    dealDetails: '48bb1f3ddac751ddcca115d1340e6a47983d3687',
    dealLeadSource: '30c4f33c7c7030fdd3360f5681851efb5de42712',
...
const handler = async event => {
  ...
  if (form_name === 'contact-update') {
    try {
      await pipedriveUpdateFunc(process, data);
      await klaviyoFunc(process, data);
    } catch (error) {
      console.error('error creating note', error);
      return { statusCode: 422, body: String(error) };
    }
  ...
  if (form_name === 'contact') {
    try {
      await pipedriveFunc(process, data);
      await klaviyoFunc(process, data);
    } catch (error) {
      console.error('error creating note', error);
      return { statusCode: 422, body: String(error) };
  ...
  if (form_name === 'newsletter') {
    try {
      await klaviyoNewsLetterFunc(process, data);
    } catch (error) {
      console.error('error creating note', error);
      return { statusCode: 422, body: String(error) };
    }
